...
def create_client_id(company):
	if company:
		...
		if response_json:
			client_id = response_json.get("_id")
			url2 = f"{self.dsfinvk_base_url}/cash_registers/{client_id}"
			payload2 = {
					"base_currency_code": "EUR",
					"brand": "Frappe",
					"model": "ERP Next",
					"cash_register_type": {"type": "MASTER", "tss_id": tss_id},
					"software": {"brand": "ERP Next", "version": "v15"},
					"processing_flags": { "UmsatzsteuerNichtErmittelbar": True},
				}
			response_json2 = make_call(url2, "PUT", headers, payload2)
			return client_id
	return False
